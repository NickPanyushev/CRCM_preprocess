...
Remy_goi_mut_table <- Remy_goi_mut_table[, .(Sample, Gene_symbol, Remy_Mutations)]
```
  
  
Let's have a look what we've got. 
```{r, echo=FALSE}
setDF(Remy_goi_mut_table)
datatable(Remy_goi_mut_table)
```
...
Exome_data_goi_mut_table <- Exome_data_goi_mut_table[, .(symbol, Sample, AA_mut_1_letter)]
Exome_data_goi_mut_table <- unique(Exome_data_goi_mut_table)
Exome_data_goi_mut_table[, Exome_Mutations := paste(AA_mut_1_letter, collapse = "; "), by = .(symbol, Sample)]
Exome_data_goi_mut_table <- unique(Exome_data_goi_mut_table[, .(symbol, Sample, Exome_Mutations)])
setnames(Exome_data_goi_mut_table, old = "symbol", new = "Gene_symbol")
```

```{r merging_Remy_and_my_results}
comparison_table <- merge(Exome_data_goi_mut_table, Remy_goi_mut_table)
