...
filtered_table <- tidyr::unnest(filtered_table, cols = (SYMBOL))
...
unique(table_for_julie$Indiv)

fwrite(unique(table_for_julie[, .(Indiv)]), file = "Samples_list.txt", col.names = F)
```
Now making the output table for Nelson, looking just like 
SAMPLE | Gene_symbol | Protein_mutation |  
```{r output_table}

output_table <- filtered_table[FILTER == "PASS", 
               .(CHROM, POS, ID, Indiv, FILTER, HGVSp, SYMBOL)]

output_table$HGVSp <- vapply(output_table$HGVSp, function(x) strsplit(as.character(x), ","), FUN.VALUE = list(character(1)))

output_table <- tidyr::unnest(output_table, cols = HGVSp)
...
setnames(output_table, c("Indiv", "SYMBOL", "HGVSp"), c("Sample", "symbol", "prot_keep"))

fwrite(output_table, "all_genes_fixed.tsv", sep = "\t")

fwrite(output_table[symbol %in% c("KRAS", "TP53", "SMAD4", "BRCA1", "BRCA2")], "KRAS_TP53_SMAD4_BRAC_genes_fixed.tsv", sep = "\t")
```
